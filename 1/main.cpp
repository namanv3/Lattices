...
int norm_order (mpz_class B[][MAXSIZE], int n, int m) {
	mpz_class minNorm = norm(B[0],m), currNorm;
	for (int i = 1; i < n; i++) {
		currNorm = norm(B[i],m);
		if (minNorm > currNorm) minNorm = currNorm;
	}
	string s = minNorm.get_str();
	return s.size();
}
...
int reduce_main (mpz_class B[][MAXSIZE], int n, int m) {
	if(n == 1) return 0;
	mpz_class temp[m];
	int order = norm_order(B,n,m);
	printf("Currently the order is %d\n\n", order);
	int count = 0;
	while (true) {
		LLL (B, n, m);
		printf("After LLL, the matrix is:\n");
		print(B,n,m);
		printf("\n");
		for (int i = 0; i < n; i++) {
		...
		print(B,n,m);
		int currOrder = norm_order(B,n,m);
		printf("Current order: %d\n", currOrder);
		if (currOrder >= order) break;
		order = currOrder;
		printf("Round %d done\n\n", count);
	}
	return count;
